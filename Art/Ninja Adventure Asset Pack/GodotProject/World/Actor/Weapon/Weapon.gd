...
func _ready():
	disable_weapon()
	visible = false
	tween.connect("tween_all_completed", Callable(self, "attack_end"))
...
func on_attack():
	...
	var py = 16
	var final_py = 10
	match(int(rad_to_deg(direction))):
		-90:
			px = -3
		90:
			px = 4
